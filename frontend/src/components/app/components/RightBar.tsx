...
import placeholder from "../../../assets/logos/png/logo-icon.png";

const Rightbar = () => {
  const [notJoinedCommunitiesFetched] =
    useState(false);
  const [publicUsersFetched] = useState(false);
  ...
  const followUserHandler = useCallback(
    async (toFollowId: string) => {
      console.log(`Following user ${toFollowId}`)
    },
    []
  );
  ...
  return (
    <div className="rightbar overflow-auto">
        ...
        <div>
          ...
          <ul className="flex flex-col gap-3 ">
            {visibleCommunities?.map((community) => (
              <li
                key={community._id}
                className="flex items-center justify-between bg-black px-2 py-1 rounded-md text-white z-10"
              >
      ...
      <ul className="flex flex-col gap-3">
        {recommendedUsers?.length > 0 &&
          recommendedUsers.map((user) => (
            <li
              key={user._id}
              className="flex justify-between items-center gap-5 bg-purple-200 shadow-2xl shadow-[#f2f5fc]  border border-slate-100 px-2 py-1 rounded-lg"
            >
